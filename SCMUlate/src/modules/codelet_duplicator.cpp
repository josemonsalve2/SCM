...
namespace scm {
  ...
  decoded_instruction_t* dupl_controller_module::duplicateCodeletInstruction(
    ...
    new_inst->setInstruction(
      std::string("DUP_") + original->first->getInstruction());
  ...
  bool dupl_controller_module::compareCodelets(instruction_state_pair* inst) {
    ...
    if (inst_buff_m->checkIfReady(inst)) {
      SCMULATE_INFOMSG(5, "Instruction %s at (0x%lX) is ready for comparison. All copies have finished",
        inst->first->getFullInstruction().c_str(), (unsigned long) inst);
      // compare
