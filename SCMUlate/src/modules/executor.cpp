...
scm::cu_executor_module::cu_executor_module(
    int CU_ID, control_store_module *const control_store_m,
    unsigned int execSlotNumber, bool *aliveSig, l2_memory_t upperMem)
    : cu_executor_id(CU_ID), aliveSignal(aliveSig) {
  this->myExecutor = control_store_m->get_executor(execSlotNumber);
  this->mem_interface_t = new mem_interface_module(upperMem, this);
}

int scm::cu_executor_module::behavior() {
...
#pragma omp atomic read
  ...
  while (alive) {
    if (!myExecutor->is_empty()) {
      SCMULATE_INFOMSG(4, "  CUMEM[%d]: Executing instruction ",
                       cu_executor_id);
      SCMULATE_ERROR_IF(
          0, myExecutor->getHead() == nullptr,
          "Executor head is NULL even though it was marked as not empty!");
      SCMULATE_INFOMSG(
          5, "  CUMEM[%d]: Executing instruction %s", cu_executor_id,
          myExecutor->getHead()->first->getFullInstruction().data());
      ...
      if (curInstruction->getType() == scm::instType::MEMORY_INST ||
          curInstruction->getExecCodelet()->isMemoryCodelet()) {
        TIMERS_COUNTERS_GUARD(
#ifdef PAPI_COUNT
            this->timer_cnt_m->startPAPIcounters(this->cu_timer_name);
#endif
            this->timer_cnt_m->addEvent(this->cu_timer_name,
                                        CUMEM_EXECUTION_MEM,
                                        curInstruction->getFullInstruction()););
        ...
        fault_injection_m.injectFault(nullptr);

        // Start timer in ms
        codeletExecutor();

        // Fault injection
        fault_injection_m.injectFault(myExecutor->getHead());

      } else {
        SCMULATE_ERROR(0,
                       "Error. Executor received an unknown instruction type");
      }
      ...
      TIMERS_COUNTERS_GUARD(
          ...
          this->timer_cnt_m->stopAndRegisterPAPIcounters(this->cu_timer_name,
                                                         anEvent);
#else
          this->timer_cnt_m->addEvent(this->cu_timer_name, CUMEM_IDLE);
#endif
