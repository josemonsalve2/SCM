...
#pragma omp barrier
  while (*(this->aliveSignal)) {
    ...
    SCMULATE_ERROR_IF(0, !new_inst, "Returned instruction is NULL for PC = %d. This should not happen", PC);
    if (!new_inst) {
    ...
    if (this->stallingInstruction == nullptr && !this->inst_buff_m.isBufferFull()) {
